...
func _draw():
	draw_line($StartPosition.position, $EndPosition.position, color, cable_width)
	draw_line($StartPosition.position, transform.xform_inv(previous_segment.get_end_position()), color, cable_width)
